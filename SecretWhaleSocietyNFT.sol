...
// File contracts/SecretWhaleSocietyNFT.sol

pragma solidity ^0.8.4;


contract SecretWhaleSocietyNFT is ERC721Enumerable, ERC2981, Ownable {
    ...
    uint256 public _price_1 = 29 * 10**18; //29 FTM;
    uint256 public _price_10 = 24 * 10**18; //24 FTM;
    uint256 public _price_50 = 19 * 10**18; //19 FTM;
    bool public _paused = true;
    address public _feeReceipient = 0xCC20c7A31172D57fDF2C1B63b10b7C3d7f9BbE4c;
    uint public constant MAX_ENTRIES = 10000;
    uint256 public constant ROYALTY_PERCENT = 8;
    address public _royaltyAddress = 0xCC20c7A31172D57fDF2C1B63b10b7C3d7f9BbE4c;
    address[] public _whiteList;

    constructor(string memory baseURI) ERC721("Secret Whale Society NFT", "SecretWhaleSocietyNFT")  {
        setBaseURI(baseURI);
    }
    ...
    function addAccountsToWhiteList(address[] memory whiteList) public onlyOwner {
    ...
    function setPrice(uint256 price_1, uint256 price_10, uint256 price_50) external onlyOwner {
        _price_1 = price_1;
        _price_10 = price_10;
        _price_50 = price_50;
    }

    function updateFeeRecipient(address feeReceipient) external onlyOwner
    {
    ...
    function mint(address to, uint256 count) public payable {
        ...
        require(count + totalSupply() <= MAX_ENTRIES, "Exceed max entries");

        if(msg.sender != owner()) {
            require(!_paused, "Sale Paused");
            require( msg.value == price * count, "Insufficient funds to mint." );

            // Send FTM fee to fee recipient
            (bool success,) = _feeReceipient.call{value : msg.value}("");
    ...
    function mintToWhiteList() public onlyOwner {
        for (uint256 i = 0; i < _whiteList.length; i++) {
            uint mintIndex = totalSupply() + 1;
            if (balanceOf(_whiteList[i]) == 0) {
                _safeMint(_whiteList[i], mintIndex);
            }
        }
    }
    ...
    function royaltyInfo(uint256 tokenId, uint256 salePrice) override external view returns (address, uint256) {
        uint256 supply = totalSupply();
        require(tokenId < supply + 1, "Incorrect token id");
