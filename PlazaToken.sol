...
contract Plaza is Context, IERC20, Ownable {
    ...
    string private _name = "Plaza Finance (Omega)";
    string private _symbol = "PLAZA";
    ...
    address payable public _devAddress =
        payable(0xD7a52A194733adb733Dad8032025a16d791cEFE5);
    ...
    function _takeDev(uint256 tPortion, uint256 rate) private {
        uint256 tDev = tPortion.mul(_devFee);
        uint256 rDev = tDev.mul(rate);
        _rOwned[_devAddress] = _rOwned[_devAddress].add(rDev);

        if (_isExcluded[_devAddress]) {
            _tOwned[_devAddress] = _tOwned[_devAddress].add(
                tDev
            );
        }
    }

    function _burnTokens(uint256 tPortion, uint256 rate) private {
        uint256 tBurning = tPortion.mul(_burningFee);
        uint256 rBurning = tBurning.mul(rate);
        _rOwned[_burnAddress] = _rOwned[_burnAddress].add(rBurning);
    ...
    function _tokenTransfer(
        ...
        if (!takeFee) {
            ...
            uint256 tPortion = tAmount.div(100);
            uint256 rate = _getRate();
            _burnTokens(tPortion, rate);
            _takeDev(tPortion, rate);
